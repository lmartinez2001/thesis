...
In this section we investigate the relationship between representations learned by 3D shape encoders and those derived from persistent homology. Our goal is to evaluate the degree of correlation between features extracted from point cloud encoders and features obtained through persistence diagrams. By doing so, we aim to understand under what circumstances both types of representations capture similar structural information. This analysis provides insight into whether learned embeddings and topological summaries align, and highlights the conditions where persistent homology can serve as a complementary tool for interpreting or validating encoder features.
...
\begin{figure}[t]
  ...
  \begin{subfigure}{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figs/ph/expers_v2.pdf}
    \caption{\textbf{Sub- (resp. super-) level set filtrations on a graph.} (Figure adapted from PersLay~\cite{perslay}) Each node of the graph is assigned its height. Persistence intervals are shown under the sequence. Figure ... highlights key differences between ordinary and extended persistence.}
  ...
  \caption{Overall caption describing how the panels relate. 
  (Figure adapted from \cite{perslay}).}
  \label{fig:filtrations}
\end{figure}
