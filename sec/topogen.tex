...
DONUT is a synthetic dataset designed to test whether 3D encoders capture topological structure rather than only geometry. Each object is a watertight manifold mesh with known numbers of connected components ($\beta_0$) and genus ($g$). This controlled setting allows us to separate geometric and topological learning.
...
\paragraph{Large object-centric datasets.}
...
where $V,E,F$ are the number of vertices, edges, and faces, $g$ is genus.
...
\paragraph{Cones.} Cones are generated using the built-in \texttt{trimesh.creation.cone} primitive. Internally, \textit{trimesh} constructs a triangulated circular base in the $xy$-plane and connects each boundary vertex to a single apex point along the $z$-axis, forming a fan of triangular faces.
...
Figure~\ref{fig:topogen-umaps-overview} shows that synthetic samples from DONUT occupy the same representation space as training data used by common 3D encoders. This indicates that the dataset lies within the learned distribution of existing models rather than being out-of-distribution. In addition, Figure~\ref{fig:topogen-gen-time}, provided in Appendix, reports the time required to generate different dataset sizes, showing that large-scale variants remain practical.
...
\label{ssec:topogen-limitations}

\paragraph{Topological Variety.} 
The current dataset has limited topological variety. It only encodes two characteristics: genus and number of connected components. This is mainly because all meshes are watertight and manifold. In this setting, topological invariants are trivial to deduce ($\beta_1 = 2 \times g$, $\beta_2=\beta_0$, see Appendix~\ref{ssec:suppl-proofs}). A more challenging extension would include surfaces with boundaries, non-watertight meshes, or nested shapes. While the current generation process allows nested components in theory, no such cases are present in practice. Another direction is to incorporate families of parametric shapes with non-trivial topology, such as catenoids, helicoids, or Möbius strips.
